...
k4a = PyK4A(Config(
    ...
    synchronized_images_only=False  # Allow color-only mode
))
    ...
    while True:
        ...
        if capture.color is not None:
            ...
            for _, row in detections.iterrows():
                ...
                # Force "vase" to be treated as "cup" (to fix misclassification)
                if label == "vase":
                    label = "cup"
                ...
                # Scale bounding boxes to match Kinect frame size
                h, w, _ = frame.shape
                x1 = int(x1 * (w / 640))
                x2 = int(x2 * (w / 640))
                y1 = int(y1 * (h / 640))
                ...
                if label == "cup":  # Mask detected cups
                    frame[y1:y2, x1:x2] = (0, 0, 0)  # Blackout the cup
                else:
                    # Draw bounding boxes for other objects
                    cv2.rectangle(frame, (x1, y1), (x2, y2), (0, 255, 0), 2)
                    cv2.putText(frame, f"{label} {row['confidence']:.2f}", (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
